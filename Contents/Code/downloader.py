...
######################################################    
  ...
  def resetArticleQueue(self):
    ...
    try:
      ...
      err, errnum, tb = sys.exc_info()
      log(1, funcName, 'Could not reset article queue', 'error:', err, 'errnum:', errnum, 'tb:', tb)
    finally:
...
#           continue
  ...
  def add_files_to_download(self, file_objs, item):
    ...
    if skippedFile and not item.failing and not self.app.unpacker_manger.get_unpacker(item.id):
...
#        pass
  ...
  def client_task(self):
    ...
    while self.notPaused and client:
      ...
      info=None
      try:
        info = self.article_queue.get(True)
    ...
    if not self.notPaused:
      if info:
        if info.decoder:
          if not info.decoder.stopped:
            info.decoder.stopped = True
            log(8, funcName, 'Stopped decoder for', info.file_obj.name)
    if info:
      if info.decoder:
        if not info.decoder.stopped:
          info.decoder.stopped = True
