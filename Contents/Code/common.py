...
CACHE_INTERVAL     = 0
TVRAGE_CACHE_TIME  = (86400 * 30)
IMDB_CACHE_TIME    = (86400 * 365)
MOVIE_GENRE_CACHE_TIME = (86400 / 24)
SEARCH_CACHE_TIME = 0
...
class NWQueue(object):
  ...
  def move(self, old_index, new_index):
    funcName = '[' + self.dictName + '.move]'
    try:
      if self.useLocking: Thread.AcquireLock(self.queueLock)
      val = self.queue[old_index]
    except:
      log(1, funcName, 'Unable to get value for index:', old_index)
    finally:
      if self.useLocking: Thread.ReleaseLock(self.queueLock)
    
    removed = self.remove(val)
    inserted = False
    if removed:
      inserted = self.insert(new_index, val)
    else:
      log(1, funcName, 'Unable to remove or insert')
    return inserted
  ...
  def insert(self, idx, val):
    ...
    try:
      ...
      log(1, 'Unable to insert value in list', self.dictName)
    finally:
      if self.useLocking: Thread.ReleaseLock(self.queueLock)
    return resp
  ...
  def save(self):
    ...
    if rightNow - self.lastSave > self.saveInterval:
      try:
        ...
        log(5, funcName, 'ERROR at step:', lastStep, ":", err)
        #raise
      finally:
...
class article(object):
  ...
  def attributes_and_summary(self):
    funcName = '[article.attributes_and_summary]'
    summary = ''
    try:
      if self.videosource != '':
    ...
    if summary != '':
      summary += '\n'
    try:
      if self.description != None: summary += self.description
    except:
      log(1, funcName, 'Could not concatenate', summary, 'and', self.description)
    return summary
...
def StupidUselessFunction(key=None, sender='nothing'):
...
####################################################################################################
def get_metadata_date(item):
  funcName = '[get_metadata_date]'
  if not item:
    return False
  metadata = {}
  report = None
  if hasattr(item, 'report'): # Received a MediaItem object
    report = item.report
    metadata = item.report.metadata
  if hasattr(item, 'metadata'): # Received an article object
    report = item
    metadata = item.metadata
  try:
    if not metadata['date']:
      log(3, funcName, 'No usable date:', report.title, ' URL:', report.moreInfoURL, ' ID:', report.moreInfo, ' Metadata:', report.metadata)
      air_release_date = dateutil.parser.parse('january 1, 1969')
    else:
      air_release_date = report.metadata['date']
  except:
    air_release_date = dateutil.parser.parse('january 1, 1969')
  return air_release_date
